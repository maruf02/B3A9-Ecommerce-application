...
const UserDashBoard = () => {
  ...
  const { data: product, isLoading } = useOrderProductByUserEmailQuery({
    email,
  });
  ...
  const totalRequiredQty = productData
    ? productData.reduce(
        (sum: number, item: TProduct) => sum + (item.requiredQty || 0),
        0
      )
    : 0;
  const totalPrice = productData
    ? productData.reduce(
        (sum: number, item: TProduct) => sum + (item.price || 0),
        0
      )
    : 0;

  // Bar Chart Data (Adjusted for visualization)
  const chartData = [
    {
      name: "Total Items",
    ...
    {
      name: "Total Quantity",
      value: totalRequiredQty * 2000,
      original: totalRequiredQty,
    },
    { name: "Total Amount", value: totalPrice, original: totalPrice },
  ];
    ...
    <div className="w-full h-full min-h-screen px-12  my-10">
      ...
      <div className="w-full h-fit flex flex-col md:flex-row lg:flex-row gap-5 justify-between   mb-20 ">
        <div>
          <div className="card w-fit shadow-xl ">
            <div className="card-body">
              <h2 className="card-title">Total Purchase Item</h2>
        ...
        <div>
          <div className="card w-fit shadow-xl">
            <div className="card-body">
              <h2 className="card-title">Total Purchase Item QTY</h2>
              <h2 className="card-title">{totalRequiredQty} pcs</h2>
            </div>
          </div>
        </div>
        <div>
          <div className="card w-fit shadow-xl">
            <div className="card-body">
              <h2 className="card-title">Total Purchase Amount</h2>
          ...
          >
            ...
            <Tooltip
              formatter={(value: number, name: string, props: any) => {
                if (!props || !props.payload) return `${value} (${name})`; // Use value and name if payload is not available

                const unit =
                  props.payload.name === "Total Amount" ? "Tk" : "pcs";
